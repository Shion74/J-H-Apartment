...
class Bill {
  ...
  // Generate monthly comprehensive bills for all tenants
  static async generateMonthlyBills(month, year) {
    try {
      // Get current rates
      const rates = await Setting.getBillingRates();
      
      // Get all tenants with active rooms
      const [tenants] = await pool.execute(`
        SELECT t.id as tenant_id, t.name, t.room_id, r.monthly_rent, r.room_number, br.name as branch_name
        FROM tenants t
      ...
      // Create comprehensive bills for each tenant
      const createdBills = [];
      
      for (const tenant of tenants) {
        // Get previous month's electric reading if available
        const [prevBill] = await pool.execute(`
        ...
        const prevElectricReading = prevBill.length > 0 ? prevBill[0].electric_present_reading : 0;
        
        const billData = {
          tenant_id: tenant.tenant_id,
          room_id: tenant.room_id,
          bill_date: billDate.toISOString().split('T')[0],
          rent_from: firstDay.toISOString().split('T')[0],
          rent_to: lastDay.toISOString().split('T')[0],
          rent_amount: tenant.monthly_rent,
          electric_present_reading: 0, // To be filled manually
          electric_previous_reading: prevElectricReading,
          electric_consumption: 0,
          electric_rate_per_kwh: rates.electric_rate_per_kwh,
          electric_amount: 0,
          water_amount: rates.water_fixed_amount,
          total_amount: tenant.monthly_rent + rates.water_fixed_amount, // Start with rent + water
          status: 'unpaid',
          notes: `Monthly bill for ${new Date(billDate).toLocaleString('default', { month: 'long' })} ${year}`,
          prepared_by: 'System'
        };
        
        const result = await this.create(billData);
